...
// Tech icon colors for particle effects
export const TECH_COLORS = ['#E44D26', '#1572B6', '#F0DB4F', '#61DAFB'] as const;
...
export function generateStartPositions(): { html: AnimationPosition; css: AnimationPosition; js: AnimationPosition } {
  const shuffled = [...START_POSITIONS].sort(() => Math.random() - 0.5);
  return {
    html: { x: shuffled[0].x, y: shuffled[0].y, rotate: Math.random() * 360 - 180, opacity: 0 },
    css: { x: shuffled[1].x, y: shuffled[1].y, rotate: Math.random() * 360 - 180, opacity: 0 },
    js: { x: shuffled[2].x, y: shuffled[2].y, rotate: Math.random() * 360 - 180, opacity: 0 },
  };
}
